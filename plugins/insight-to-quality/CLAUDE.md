...
flowchart TD
    ...
    FC --> GH[ec:gherkin]
    GH --> TDD[ec:tdd-workflow<br/>with Verification Ledger]
    TDD --> DR[ec:design-review]
    DR --> PC[ec:pre-complete<br/>with Delta Spec sync]
...
- **Discovery is a prerequisite**: Proceeding to implementation without goals.md, dominant-ops.md, and SYSTEM_MAP.md will cause ec:feature-planning to block and require discovery first
...
- **Complete one spec before starting the next**: Walk through ec:feature-planning → ec:pre-complete fully before starting another spec
...
- **Wait for user confirmation**: ec:feature-coverage analysis, ec:tdd-workflow Verification Ledger sign-off, and red-light confirmation all require explicit user approval before proceeding
...
| From | To | Handoff |
|------|----|---------|
| ec:goals-discovery | ec:dominant-ops | After goals.md is confirmed, use Gx IDs as the traceability anchor |
| ec:dominant-ops | ec:system-map | After dominant-ops.md is confirmed, Dx + Anti-Patterns drive boundary design |
| ec:system-map | ec:align-internals | SYSTEM_MAP's Boundary Map drives contract alignment |
| ec:system-map | ec:align-surface | SYSTEM_MAP's Component Map + Dx user journeys drive interface alignment |
| ec:align-internals/surface | ec:feature-planning | After alignment, read SYSTEM_MAP gaps to decide the next feature |
| ec:feature-planning | ec:feature-coverage | After the feature plan is established, proceed to coverage analysis |
| ec:feature-coverage | ec:gherkin | After coverage analysis is confirmed, trigger ec:gherkin to write the .feature file |
| ec:gherkin | ec:tdd-workflow | After the .feature file is written, create the Verification Ledger first, then proceed to Red |
| ec:tdd-workflow | ec:design-review | After green + refactor, remind the user to trigger ec:design-review |
| ec:design-review | ec:pre-complete | After review, trigger ec:pre-complete (with delta spec sync) before commit/PR |
...
mapping to the Goal IDs in goals.md. A change without a `Serves:` field has no goal foundation — ec:feature-planning and ec:feature-coverage will block it and require the field to be added.
